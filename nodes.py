        ...
        for jf in glob.glob(os.path.join(subf_dir, "*.json")):
            ...
            if not os.path.exists(dest):
                try:
                    ...
                    print(f"[ZImageOneNode] migrate sidecar {basename}: {e}")
            else:
                try:
                    os.remove(jf)
                except Exception:
                    pass
        if moved:
            print(f"[ZImageOneNode] Migrated {moved} metadata sidecar(s) to metadata/")
    except Exception as e:
        print(f"[ZImageOneNode] migrate_meta_sidecars error: {e}")
...
def _png_embed_meta(png_path, meta_dict):
    ...
    try:
        ...
        print(f"[ZImageOneNode] png_embed_meta error: {e}")
        return False
...
def _png_read_meta(png_path):
    ...
    try:
        ...
        print(f"[ZImageOneNode] png_read_meta error: {e}")
        return None
...
def _read_json_meta(image_path):
    ...
    for mp in (_meta_path(image_path), _meta_path_legacy(image_path)):
        ...
        try:
            ...
            print(f"[ZImageOneNode] read_json_meta error: {e}")
    return None
...
def _write_json_meta(image_path, meta_dict):
    ...
    if image_path.lower().endswith('.png') and os.path.exists(image_path):
        ...
        if ok_png:
            try:
                ...
                pass
            print(f"[ZImageOneNode] Meta embedded in PNG: {os.path.basename(image_path)}")
    ...
    try:
        ...
        print(f"[ZImageOneNode] write_json_meta error: {e}")
...
PromptServer.instance.routes.get("/z_image/workflow_t2i")(_serve_json("workflows/t2i_workflow.json"))
PromptServer.instance.routes.get("/z_image/workflow_i2i")(_serve_json("workflows/i2i_workflow.json"))


@PromptServer.instance.routes.get("/z_image/bgremoval_models")
...
@PromptServer.instance.routes.get("/z_image/config")
...
@PromptServer.instance.routes.post("/z_image/config")
async def save_config_route(request):
    try:
        ...
        print(f"[ZImageOneNode] config save error: {e}")
        return web.json_response({"ok": False, "error": str(e)}, status=500)


@PromptServer.instance.routes.get("/z_image/gallery")
...
@PromptServer.instance.routes.post("/z_image/save_meta")
async def save_meta(request):
    try:
        ...
        print(f"[ZImageOneNode] save_meta error: {e}")
        return web.json_response({"ok": False, "error": str(e)})


@PromptServer.instance.routes.post("/z_image/update_meta")
async def update_meta(request):
    try:
        ...
        print(f"[ZImageOneNode] update_meta error: {e}")
        return web.json_response({"ok": False, "error": str(e)})


@PromptServer.instance.routes.get("/z_image/meta")
...
@PromptServer.instance.routes.post("/z_image/open_folder")
...
@PromptServer.instance.routes.post("/z_image/delete")
...
@PromptServer.instance.routes.get("/z_image/models")
...
@PromptServer.instance.routes.get("/z_image/lora_triggers")
...
NODE_CLASS_MAPPINGS = {"ZImageOneNode": ZImageOneNode}
NODE_DISPLAY_NAME_MAPPINGS = {"ZImageOneNode": "One Node · Z-Image"}
